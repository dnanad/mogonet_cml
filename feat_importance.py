import os
import copy
import numpy as np
import pandas as pd
import torch

from sklearn.metrics import f1_score
from utils import load_model_dict
from models import init_model_dict
from train_test import prepare_trte_data, gen_trte_adj_mat, test_epoch

cuda = True if torch.cuda.is_available() else False


def cal_feat_imp(
    data_folder, exp_path, view_list, num_class, adj_parameter, dim_he_list
):
    """Calculate feature importance

    Parameters
    ----------
    data_folder : str
        Folder name of the dataset
    exp_path : str
        Folder name for the experiment
    view_list : list
        List of views
    num_class : int
        Number of classes
    adj_parameter : int
        Number of neighbors for each node
    dim_he_list : list
        List of hidden dimensions for each view

    Returns
    -------
    featimp_list : list
        List of important feature for each view
    """
    num_view = len(view_list)
    dim_hvcdn = pow(num_class, num_view)
    # if data_folder == "TEST_DATA":
    #     adj_parameter = 2
    #     dim_he_list = [400, 400, 200]
    # if data_folder == "ROSMAP":
    #     adj_parameter = 2
    #     dim_he_list = [200, 200, 100]
    # if data_folder == "BRCA":
    #     adj_parameter = 10
    #     dim_he_list = [400, 400, 200]

    # prepare data
    data_tr_list, data_trte_list, trte_idx, labels_trte = prepare_trte_data(
        data_folder, view_list
    )

    # generate adjacent matrix
    adj_tr_list, adj_te_list = gen_trte_adj_mat(
        data_tr_list, data_trte_list, trte_idx, adj_parameter
    )
    featname_list = []

    # load feature names
    for v in view_list:
        df = pd.read_csv(
            os.path.join(data_folder, str(v) + "_featname.csv"), header=None
        )
        featname_list.append(df.values.flatten())

    dim_list = [x.shape[1] for x in data_tr_list]
    # initialize model
    model_dict = init_model_dict(num_view, num_class, dim_list, dim_he_list, dim_hvcdn)
    for m in model_dict:
        if cuda:
            model_dict[m].cuda()
    # load model
    model_dict = load_model_dict(exp_path, model_dict)
    te_prob = test_epoch(data_trte_list, adj_te_list, trte_idx["te"], model_dict)
    if num_class == 2:
        f1 = f1_score(labels_trte[trte_idx["te"]], te_prob.argmax(1))
    else:
        f1 = f1_score(labels_trte[trte_idx["te"]], te_prob.argmax(1), average="macro")

    feat_imp_list = []

    # calculate feature importance
    for i in range(len(featname_list)):
        feat_imp = {"feat_name": featname_list[i]}
        feat_imp["imp"] = np.zeros(dim_list[i])  # add zeros
        for j in range(dim_list[i]):
            feat_tr = data_tr_list[i][:, j].clone()
            feat_trte = data_trte_list[i][:, j].clone()
            data_tr_list[i][:, j] = 0
            data_trte_list[i][:, j] = 0
            # generate adjacent matrix again foe each feature
            adj_tr_list, adj_te_list = gen_trte_adj_mat(
                data_tr_list, data_trte_list, trte_idx, adj_parameter
            )
            # test the model with one feature removed from each view and calculate the difference in f1 score
            te_prob = test_epoch(
                data_trte_list, adj_te_list, trte_idx["te"], model_dict
            )
            if num_class == 2:
                f1_tmp = f1_score(labels_trte[trte_idx["te"]], te_prob.argmax(1))
            else:
                f1_tmp = f1_score(
                    labels_trte[trte_idx["te"]], te_prob.argmax(1), average="macro"
                )
            feat_imp["imp"][j] = (f1 - f1_tmp) * dim_list[
                i
            ]  # calculate the difference in f1 score and multiply by the number of features
            # reset the feature
            data_tr_list[i][:, j] = feat_tr.clone()
            data_trte_list[i][:, j] = feat_trte.clone()
        feat_imp_list.append(pd.DataFrame(data=feat_imp))

    return feat_imp_list


def summarize_imp_feat(featimp_list_list, exp_path, topn, feat_file_suffix):
    """Summarize important features for each view

    Parameters
    ----------
    featimp_list_list : list
        List of important features for each view
    topn : int
        Number of top features to be summarized for each view

    Returns
    -------
    df_featimp : pandas.DataFrame
        Dataframe of important features for each view
    """
    num_rep = len(featimp_list_list)
    num_view = len(featimp_list_list[0])
    df_tmp_list = []
    for v in range(num_view):
        df_tmp = copy.deepcopy(featimp_list_list[0][v])
        df_tmp["omics"] = np.ones(df_tmp.shape[0], dtype=int) * v
        df_tmp_list.append(df_tmp.copy(deep=True))
    df_featimp = pd.concat(df_tmp_list).copy(deep=True)
    for r in range(1, num_rep):
        for v in range(num_view):
            df_tmp = copy.deepcopy(featimp_list_list[r][v])
            df_tmp["omics"] = np.ones(df_tmp.shape[0], dtype=int) * v
            df_featimp = pd.concat([df_featimp, df_tmp], ignore_index=True)
            # df_featimp = df_featimp.append(df_tmp.copy(deep=True), ignore_index=True)
    df_featimp_top = df_featimp.groupby(["feat_name", "omics"])["imp"].sum()
    df_featimp_top = df_featimp_top.reset_index()
    df_featimp_top = df_featimp_top.sort_values(by="imp", ascending=False)
    df_featimp_top = df_featimp_top.iloc[:topn]
    print("{:}\t{:}".format("Rank", "Feature name"))
    for i in range(len(df_featimp_top)):
        print("{:}\t{:}".format(i + 1, df_featimp_top.iloc[i]["feat_name"]))
    # save the df_featimp_top at the exp_path
    df_featimp_top.to_csv(
        os.path.join(exp_path, "featimp_" + feat_file_suffix + ".csv"), index=False
    )
    df_featimp_top.drop(columns=["omics", "imp"], inplace=True)
    df_featimp_top.to_csv(
        os.path.join(exp_path, "processed_featimp_" + feat_file_suffix + ".csv"),
        index=False,
    )
